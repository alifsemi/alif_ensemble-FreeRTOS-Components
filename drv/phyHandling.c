                                     ...
                                     uint32_t ulPhyMask )
{
    ...
    for( ; ; )
    {
        ...
        if( xTaskCheckForTimeOut( &xTimer, &xRemainingTime ) != pdFALSE )
        {
            FreeRTOS_printf( ( "xPhyStartAutoNegotiation: phyBMCR_RESET timed out ( done 0x%02X )\n", ulDoneMask ) );
            break;
        }
...
/*	[010] = 100BASE-TX half-duplex */
            ...
            else if( ulPhyID == PHY_ID_RTL8201 )
            {
                /* For the RTL8201 PHYs we read the link information from BMCR */
                uint32_t ulControlStatus;

                pxPhyObject->fnPhyRead( xPhyAddress, phyREG_00_BMCR, &ulControlStatus );
                ulRegValue = 0;

                if( ( ulControlStatus & phyBMCR_FULL_DUPLEX ) != 0)
                {
                    /* full dulex */
                    ulRegValue |= phyPHYSTS_DUPLEX_STATUS;
                }

                if( ( ulControlStatus & phyBMCR_SPEED_100 ) == 0)
                {
                    /* 10mbps */
                    ulRegValue |= phyPHYSTS_SPEED_STATUS;
                }
                                ...
                                BaseType_t xHadReception )
{
    ...
    else if( xTaskCheckForTimeOut( &( pxPhyObject->xLinkStatusTimer ), &( pxPhyObject->xLinkStatusRemaining ) ) != pdFALSE )
    {
        ...
        for( xPhyIndex = 0; xPhyIndex < pxPhyObject->xPortCount; xPhyIndex++, ulBitMask <<= 1 )
        {
            ...
            if( pxPhyObject->fnPhyRead( xPhyAddress, phyREG_01_BMSR, &ulStatus ) == pdPASS )
            {
